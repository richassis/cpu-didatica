...
export default function MainMemoryComponent({ component, zoom }: Props) {
  ...
  const mem       = useSimulatorStore((s) => s.getMainMemory(id));
  ...
  const conflict = rdMem !== 0 && wrMem !== 0; // Both signals active (write takes priority)
    ...
    <>
      <div
        ...
        className={`select-none cursor-grab active:cursor-grabbing relative rounded-xl border-2 ${
          accessing ? "shadow-lg shadow-purple-900/40" : ""
        } ${borderCls} bg-gray-950/90`}
        onDoubleClick={(e) => { e.stopPropagation(); setConfigOpen(true); }}
      >
        {/* ── Header ── */}
        <div className="flex items-center justify-between px-2 py-1 bg-purple-900/70 border-b border-purple-700/40">
          <span className="text-[11px] font-bold text-purple-200 truncate leading-none">
            {label}
          </span>
          <div className="flex items-center gap-1">
            ...
            {viewMode === "data" && (
              <button
                ...
                className={`text-[10px] leading-none px-1 rounded transition-colors ${
                  editMode
                    ? "bg-purple-500 text-black"
                    : "text-purple-300/50 hover:text-purple-100"
                }`}
            ...
            <button
              ...
              className="text-purple-300/50 hover:text-white text-[10px] leading-none ml-1"
              aria-label="Remove"
            >✕</button>
          </div>
        </div>
        ...
        {viewMode === "ports" ? (
          ...
          <div className="flex flex-col gap-1.5 px-2 py-2 text-[10px] font-mono" style={{ height: h - 28 }}>
            {/* RD/WR badges with conflict indicator */}
            <div className="flex items-center justify-center gap-2 mb-1">
              <span className={`text-[9px] font-mono px-2 py-0.5 rounded ${
                rdMem ? (conflict ? "bg-red-500 text-black font-bold" : "bg-purple-500 text-black font-bold") : "bg-gray-700 text-gray-400"
              }`}>RD</span>
              <span className={`text-[9px] font-mono px-2 py-0.5 rounded ${
                wrMem ? (conflict ? "bg-red-500 text-black font-bold" : "bg-purple-500 text-black font-bold") : "bg-gray-700 text-gray-400"
              }`}>WR</span>
            </div>

            {conflict && (
              <div className="text-[8px] text-red-400 text-center bg-red-900/30 rounded px-1 py-0.5 mb-1">
                ⚠ Conflict: Write priority
              </div>
            )}

            {/* Address */}
            <div className="flex items-center justify-between">
              <span className="text-purple-400/70 uppercase tracking-wide text-[9px]">addr</span>
              <span className="text-purple-100 bg-purple-900/50 rounded px-1 py-px">
                {addrFmt}
              </span>
            </div>
            ...
            {/* Data-in */}
            <div className="flex items-center justify-between">
              <span className="text-purple-400/70 uppercase tracking-wide text-[9px]">data in</span>
              <span className={`rounded px-1 py-px ${
                wrMem ? "text-purple-100 bg-purple-900/60" : "text-gray-500 bg-gray-800/60"
              }`}>
                {dataInFmt}
              </span>
            </div>
            ...
            {/* Data-out */}
            <div className="flex items-center justify-between">
              <span className="text-purple-300/70 uppercase tracking-wide text-[9px]">data out</span>
              <span className={`rounded px-1 py-px ${
                rdMem && !conflict ? "text-purple-100 bg-purple-800/70 font-bold" : "text-gray-500 bg-gray-800/60"
              }`}>
                {dataOutFmt}
              </span>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-center">
              ...
              <div className={`text-center text-sm font-mono rounded py-1 ${
                accessing ? "bg-purple-900/50 text-purple-100" : "bg-gray-800/50 text-gray-400"
              }`}>
          ...
          /* Default data view: showing addresses and values */
...
function MemoryDataRow({ addr, value, bitWidth, addrBits, base, isActive }: DataRowProps) {
  ...
  return (
    <div className={`flex items-center justify-between py-0.5 px-0.5 rounded ${
      ...
      <span className={`text-[9px] font-mono ${isActive ? "text-purple-300 font-semibold" : "text-gray-600"}`}>
        {addrHex(addr, addrBits)}
      </span>
      <span className={`text-[10px] font-mono ${isActive ? "text-purple-100" : "text-gray-400"}`}>
        {displayed}
      </span>
    </div>
  );
...
function MemoryCellRow({ addr, value, bitWidth, addrBits, base, isActive, onPoke }: CellRowProps) {
  ...
  return (
    <div className={`flex items-center gap-1 py-px px-0.5 rounded ${
      ...
      <span className={`shrink-0 text-[9px] ${isActive ? "text-purple-300" : "text-gray-600"}`}>
        {addrHex(addr, addrBits)}
      </span>
      <input
        ref={inputRef}
        ...
        className={`flex-1 min-w-0 rounded text-[9px] font-mono px-1 py-px focus:outline-none
                    border ${isActive
                      ? "bg-purple-900/60 border-purple-600/60 text-purple-100 focus:border-purple-400"
