...
/**
 * Main Memory - Memory with exclusive read OR write operations.
 *
 * A word-addressable memory with configurable size and word width.
 * Unlike the unified Memory component, MainMemory enforces exclusive access:
 * it can either read OR write in a single cycle, but not both.
 *
 * Port map:
 *  Inputs:
 *    addr    – address to read from or write to
 *    data    – data to write (only used when wrMem = 1)
 ...
 *
 * If both rdMem and wrMem are 1 simultaneously, wrMem takes priority
 * and the read is ignored (no output update).
 *
 * Default: 256 words × 16 bits.
 */
export class MainMemory implements Clockable, Connectable {
  /** Unique ID matching the ComponentInstance id on the canvas. */
  readonly id: string;
  /** Human-readable name, e.g. "RAM". */
  name: string;
  ...
  /**
   * Evaluate one memory access cycle with exclusive read/write.
   *
   * If wrMem is 1 → write data to mem[addr] (read is ignored).
   * Else if rdMem is 1 → drive out_data with mem[addr].
   * If rdMem is 0 → out_data is held at its previous value (bus hold).
   *
   * Write takes priority over read when both signals are active.
   */
  execute(): void {
    ...
    if (wr) {
      // Write takes priority - perform write and skip read
      this._cells[addr] = this.clampWord(this.in_data.value);
    } else if (rd) {
      // Only read if not writing
